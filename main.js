...
app.listen(3000, () => {
  console.info('Talent hits a target no one else can hit; Genius hits a target no one else can see.');
});
  ...
  request('https://www.treasury.gov/resource-center/data-chart-center/interest-rates/pages/textview.aspx?data=yield', function (error, response, html) {
    if (!error && response.statusCode == 200) {
      ...
      $('.t-chart').filter(function(){
        ...
        // create random integer to trick twitter API -- currently disabled
